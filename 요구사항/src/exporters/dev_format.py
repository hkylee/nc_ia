...
class PolicyHTMLParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag in ("h1", "h2", "h3", "h4"):
            ...
            elif "policy-item-title" in classes and self._in_policy_item:
                self._in_policy_item_title = True
                self._buf = []
            elif "policy-item-content" in classes and self._in_policy_item:
    ...
    def handle_endtag(self, tag: str) -> None:
        if tag == "h1":
            ...
            if self._h3 == "나. 정책 상세":
                m = re.search(r"\(PG-[A-Z0-9-]+\)", self._h4)
                if m:
                    self._current_pg_in_detail = m.group(0).strip("()")
            ...
            if self._in_policy_item_title:
                ...
                self._pi_content_buf = list(self._buf or [])
                self._buf = None
            elif self._in_policy_item:
                # closing the policy-item wrapper — finalize an item
    ...
    def _finalize_policy_item(self) -> None:
        ...
        if not m:
            return
        pi_id = m.group(1)
        name = re.sub(rf"\s*\({PI_PREFIX_RE}\)\s*", "", title_text).strip()
        name = re.sub(r"^[•·\-\*]\s*", "", name).strip()
        self.policy_detail_items.append(PolicyDetailItem(
            pi_id=pi_id,
...
def _column(row: list[Cell], headers: list[str], header_name: str) -> Cell | None:
    ...
    for i, h in enumerate(headers):
        if h == header_name and i < len(row):
            return row[i]
    return None
...
def classify_table(t: Table) -> str:
    ...
    if "용어 ID" in h and "용어" in h:
        return "terms"
    if "액터 ID" in h and ("액터명" in h or "액터" in h):
        return "actors"
    if "유즈케이스 ID" in h and "유즈케이스명" in h:
        return "usecases"
    if "상태 코드" in h and "상태명" in h:
        return "states"
    if "현재 상태" in h and "다음 상태" in h:
        return "transitions"
    if "프로세스 ID" in h and "프로세스명" in h:
        return "processes"
    # process detail table — vertical layout ([항목, 내용]), first row "프로세스 ID".
    # Carries actor / related-fn / related-pg cells as named-text only.
    if h == {"항목", "내용"} and t.rows and t.rows[0] and t.rows[0][0].text.strip() == "프로세스 ID":
    ...
    if h == {"항목", "내용"} and t.rows and t.rows[0] and t.rows[0][0].text.strip() == "기능 ID":
        return "function_detail"
    if "기능 ID" in h and "기능명" in h:
        return "functions"
    # 정책 목록: 정책 ID, 정책명, 설명, 정책 항목
    if "정책 ID" in h and "정책명" in h and "정책 항목" in h:
        return "policy_list"  # policy_groups (with embedded item ids)
    # 정책 상세: 정책 항목, 정책 ID 등 (다른 구조)
    if "정책 항목" in h and ("정책 ID" in h):
...
def _extract_functions(t: Table, ents: Entities) -> None:
    ...
    for row in t.rows:
        ...
        if pr_from_h4:
            proc = ents.processes.get(pr_from_h4) or Process(id=pr_from_h4)
            if eid not in proc.related_function_ids:
                proc.related_function_ids.append(eid)
            ents.processes[pr_from_h4] = proc
